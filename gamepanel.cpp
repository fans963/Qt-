...
void GamePanel::resizeEvent(QResizeEvent *event)
{
    ...
    for (auto it = m_contextMap.begin(); it != m_contextMap.end(); ++it) {
        PlayerContext &context = it.value();
        Player *player = it.key();

        // 获取玩家卡牌数量
        int cardCount = player->getCards().cardCount();
        int cardWidth = m_cardSize.width();
        int cardHeight = m_cardSize.height();
        int cardSpace = 20;

        if (context.align == Horizontal) {
            if (player == m_gameCtl->getUserPlayer()) {
                // 用户玩家（底部）
                int totalWidth = cardWidth * cardCount + (cardCount - 1) * cardSpace;
                context.cardRect = QRect((windowWidth - totalWidth) / 2, windowHeight - cardHeight - 50, totalWidth, cardHeight);
            }
            else if (player == m_gameCtl->getLeftRobot()) {
                // 左侧机器人玩家
                context.cardRect = QRect(50, (windowHeight - cardHeight) / 2, cardWidth, cardHeight);
            }
            else if (player == m_gameCtl->getRightRobot()) {
                // 右侧机器人玩家
                context.cardRect = QRect(windowWidth - cardWidth - 50, (windowHeight - cardHeight) / 2, cardWidth, cardHeight);
            }
        }
        ...
        QPoint roleImgPos;
        if (player == m_gameCtl->getUserPlayer()) {
            roleImgPos = context.cardRect.center() + QPoint(250, -context.cardRect.height() / 2 - 20); // 用户在底部
        }
        else if (player == m_gameCtl->getLeftRobot()) {
            roleImgPos = context.cardRect.bottomLeft() + QPoint(-80, -context.roleImg->height()-400); // 左侧机器人
        }
        else if (player == m_gameCtl->getRightRobot()) {
            roleImgPos = context.cardRect.bottomRight() + QPoint(80 - context.roleImg->width(), -context.roleImg->height()-400); // 右侧机器人
        }
        context.roleImg->move(roleImgPos);

        // 更新信息区域位置
        QPoint infoPos(context.cardRect.left() + (context.cardRect.width() - context.info->width()) / 2,
                       context.cardRect.top() + (context.cardRect.height() - context.info->height()) / 2);
        context.info->move(infoPos);
    ...
    // 更新游戏场景中的卡牌面板位置
    m_baseCardPos = QPoint((windowWidth - m_cardSize.width()) / 2, windowHeight / 2 - 100);
    m_baseCard->move(m_baseCardPos);
    m_moveCard->move(m_baseCardPos);

    // 更新最后三张牌的位置
    int base = (windowWidth - 3 * m_cardSize.width() - 2 * 10) / 2;
    for (int i = 0; i < 3; ++i) {
        m_last3Card[i]->move(base + (m_cardSize.width() + 10) * i, 20);
    }
}
